...
def generate_counter(topic, user_claim, position):
    ...
    -ディベート（反論）の流れは以下の通りです
        ...
        2.\n **ユーザーの一つ目のポイントへの反論**: - ユーザーの主張:{user_claim}に具体的に反論してください。あなたは{position}側です。\n
        3.\n **ユーザー一つ目のポイントへの再反論**: - ユーザーからの反論を再反論してください。\n
        4.\n**二つ目のポイントの説明**: - 現在の状況とその問題点、そしてその解決がもたらす利益を説明してください。\n 
        5. \n**結論**: あなたの主張を再度述べてください。\n
    -制約条件
...
def generate_final(topic, user_claim, user_counter, position):
    ...
    以下の流れで出力してください。
    1. **結論**: あなたの主張を簡潔に述べてください。 \n
    2. \n**ディベートのまとめ**: 以下の要素を含めてまとめてください。\n
        - あなたが重要だと思う点 
        - ユーザー側の{position}についての意見 
        - なぜあなたの{position}側の意見が優れているのか 
    3. \n**結論**: あなたの主張をもう一度述べてください。 
